...
void arr_ij(int* sizeRow, int* sizeCol, int** pArr) {
	...
	int cnt = 1;
	// 2차원 배열 만들기.
	int** ary = (int**)malloc(sizeof(int) * (*sizeRow) * (*sizeCol));
	for (int i = 0; i < (*sizeRow); i++) {
		ary[i] = (int*)malloc(sizeof(int) * (*sizeCol));
	}
	...
	//	*(pArr + i) = (int*)calloc(*sizeCol, sizeof(int));
	//}
